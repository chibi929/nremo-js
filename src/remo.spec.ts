    ...
    describe('正常系', () => {
      ...
      it('ユーザーの更新情報が正しく POST できること', () => {
        return client.updateMe('hoge').then(res => {
          assert.equal(res.id, "test_id");
          assert.equal(res.nickname, "hoge");
        });
      });
    });
    ...
    describe('正常系', () => {
      ...
      it('デバイス情報配列を返すこと', () => {
        return client.fetchDevices().then(res => {
          ...
          assert.equal(res[0].id, 'test_id');
          assert.equal(res[0].name, 'test_name');
          assert.equal(res[0].temperature_offset, 123);
          assert.equal(res[0].humidity_offset, 123);
          assert.equal(res[0].created_at, 'test_created_at');
          assert.equal(res[0].updated_at, 'test_updated_at');
          assert.equal(res[0].firmware_version, 'test_firmware_version');
          assert.equal(res[0].newest_events.te.value, 123);
          assert.equal(res[0].newest_events.te.created_at, 'test_created_at');
          assert.equal(res[0].newest_events.hu.value, 123);
          assert.equal(res[0].newest_events.hu.created_at, 'test_created_at');
        });
      });
    });
    ...
    describe('正常系', () => {
      ...
      it('赤外線信号が正しく POST できること', () => {
        ...
        return client.findAirCon(message).then(res => {
          assert.equal(res.length, 1);
          assert.equal(res[0].model.id, 'test_id');
          assert.equal(res[0].model.manufacturer, 'test_manufacturer');
          assert.equal(res[0].model.remote_name, 'test_remote_name');
          assert.equal(res[0].model.name, 'test_name');
          assert.equal(res[0].model.image, 'test_image');
          assert.equal(res[0].params.temp, 'test_temp');
          assert.equal(res[0].params.vol, 'test_vol');
          assert.equal(res[0].params.dir, 'test_dir');
          assert.equal(res[0].params.mode, 'test_mode');
          assert.equal(res[0].params.button, 'test_button');
        });
      });
    });
    ...
    describe('正常系', () => {
      beforeEach(() => {
        mock.onPost('/appliances/1/aircon_settings').reply(req => {
          const params = qs.parse(req.data);
          assert.equal(params.temperature, 't');
          assert.equal(params.operation_mode, 'o');
          assert.equal(params.air_volume, 'v');
          assert.equal(params.air_direction, 'd');
          assert.equal(params.button, 'b');
          return [200, {}];
        });
      });
    ...
    describe('正常系', () => {
      ...
      it('家電の情報配列を返すこと', () => {
        return client.fetchAppliances().then(res => {
          assert.equal(res.length, 1);
          assert.equal(res[0].id, 'test_id');
          assert.equal(res[0].device.id, 'test_id');
          assert.equal(res[0].device.name, 'test_name');
          assert.equal(res[0].device.temperature_offset, 123);
          assert.equal(res[0].device.humidity_offset, 123);
          assert.equal(res[0].device.created_at, 'test_created_at');
          assert.equal(res[0].device.updated_at, 'test_updated_at');
          assert.equal(res[0].device.firmware_version, 'test_firmware_version');
          assert.equal(res[0].model.id, 'test_id');
          assert.equal(res[0].model.manufacturer, 'test_manufacturer');
          assert.equal(res[0].model.remote_name, 'test_remote_name');
          assert.equal(res[0].model.name, 'test_name');
          assert.equal(res[0].model.image, 'test_image');
          assert.equal(res[0].nickname, 'test_nickname');
          assert.equal(res[0].image, 'test_image');
          assert.equal(res[0].type, 'test_type');
          assert.equal(res[0].settings.temp, 'test_temp');
          assert.equal(res[0].settings.vol, 'test_vol');
          assert.equal(res[0].settings.dir, 'test_dir');
          assert.equal(res[0].settings.mode, 'test_mode');
          assert.equal(res[0].settings.button, 'test_button');
          assert.equal(res[0].aircon.range.modes.cool.temp, 'test_temp');
          assert.equal(res[0].aircon.range.modes.cool.vol, 'test_vol');
          assert.equal(res[0].aircon.range.modes.cool.dir, 'test_dir');
          assert.equal(res[0].aircon.range.modes.warm.temp, 'test_temp');
          assert.equal(res[0].aircon.range.modes.warm.vol, 'test_vol');
          assert.equal(res[0].aircon.range.modes.warm.dir, 'test_dir');
          assert.equal(res[0].aircon.range.modes.dry.temp, 'test_temp');
          assert.equal(res[0].aircon.range.modes.dry.vol, 'test_vol');
          assert.equal(res[0].aircon.range.modes.dry.dir, 'test_dir');
          assert.equal(res[0].aircon.range.modes.blow.temp, 'test_temp');
          assert.equal(res[0].aircon.range.modes.blow.vol, 'test_vol');
          assert.equal(res[0].aircon.range.modes.blow.dir, 'test_dir');
          assert.equal(res[0].aircon.range.modes.auto.temp, 'test_temp');
          assert.equal(res[0].aircon.range.modes.auto.vol, 'test_vol');
          assert.equal(res[0].aircon.range.modes.auto.dir, 'test_dir');
          assert.deepEqual(res[0].aircon.range.fixedButtons, ['a', 'b', 'c']);
          assert.equal(res[0].aircon.tempUnit, 'test_tempUnit');
          assert.equal(res[0].signals.length, 1);
          assert.equal(res[0].signals[0].id, 'test_id');
          assert.equal(res[0].signals[0].name, 'test_name');
          assert.equal(res[0].signals[0].image, 'test_image');
        });
      });
    });
    ...
    describe('正常系', () => {
      ...
      it('家電情報が正しく POST できること', () => {
        return client.createAppliance('1', 'hoge', 'foo', 'bar').then(res => {
          assert.equal(res.id, 'test_id');
          assert.equal(res.device.id, 'test_id');
          assert.equal(res.device.name, 'test_name');
          assert.equal(res.device.temperature_offset, 123);
          assert.equal(res.device.humidity_offset, 123);
          assert.equal(res.device.created_at, 'test_created_at');
          assert.equal(res.device.updated_at, 'test_updated_at');
          assert.equal(res.device.firmware_version, 'test_firmware_version');
          assert.equal(res.model.id, 'test_id');
          assert.equal(res.model.manufacturer, 'test_manufacturer');
          assert.equal(res.model.remote_name, 'test_remote_name');
          assert.equal(res.model.name, 'test_name');
          assert.equal(res.model.image, 'test_image');
          assert.equal(res.nickname, 'test_nickname');
          assert.equal(res.image, 'test_image');
          assert.equal(res.type, 'test_type');
          assert.equal(res.settings.temp, 'test_temp');
          assert.equal(res.settings.vol, 'test_vol');
          assert.equal(res.settings.dir, 'test_dir');
          assert.equal(res.settings.mode, 'test_mode');
          assert.equal(res.settings.button, 'test_button');
          assert.equal(res.aircon.range.modes.cool.temp, 'test_temp');
          assert.equal(res.aircon.range.modes.cool.vol, 'test_vol');
          assert.equal(res.aircon.range.modes.cool.dir, 'test_dir');
          assert.equal(res.aircon.range.modes.warm.temp, 'test_temp');
          assert.equal(res.aircon.range.modes.warm.vol, 'test_vol');
          assert.equal(res.aircon.range.modes.warm.dir, 'test_dir');
          assert.equal(res.aircon.range.modes.dry.temp, 'test_temp');
          assert.equal(res.aircon.range.modes.dry.vol, 'test_vol');
          assert.equal(res.aircon.range.modes.dry.dir, 'test_dir');
          assert.equal(res.aircon.range.modes.blow.temp, 'test_temp');
          assert.equal(res.aircon.range.modes.blow.vol, 'test_vol');
          assert.equal(res.aircon.range.modes.blow.dir, 'test_dir');
          assert.equal(res.aircon.range.modes.auto.temp, 'test_temp');
          assert.equal(res.aircon.range.modes.auto.vol, 'test_vol');
          assert.equal(res.aircon.range.modes.auto.dir, 'test_dir');
          assert.deepEqual(res.aircon.range.fixedButtons, ['a', 'b', 'c']);
          assert.equal(res.aircon.tempUnit, 'test_tempUnit');
          assert.equal(res.signals.length, 1);
          assert.equal(res.signals[0].id, 'test_id');
          assert.equal(res.signals[0].name, 'test_name');
          assert.equal(res.signals[0].image, 'test_image');
        });
      });
    });
